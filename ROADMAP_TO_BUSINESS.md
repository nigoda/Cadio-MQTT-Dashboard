...
- [x] **AI Configuration UI**:
    - [x] Add an AI Settings toggle block next to the AI Agronomist section.
    - [x] Expandable panel with numeric inputs for Rain, ET0, Temperature, and Wind speed thresholds.
    - [x] Custom rules text area for soil/plant conditions.
- [x] **AI Recommendation Generator**:
    - [x] "Suggest via AI" button that calls Gemini on the backend.
    - [x] Predict optimal numeric thresholds and natural rules based on the zone Name & Description.
    - [x] Auto-populate fields on request for user customization.
- [x] **Dynamic Backend Prompt Builder**:
    - [x] Modify `ai_agent.py` to accept and enforce dynamic parameters in the Gemini system guidelines.
    - [x] Store parameters in `config_json` inside the `automations` SQLite table.
